...
double Vec4::Dot4(const Vec4 & u, const Vec4 & v)
{
	double total = 0.0;
	for (int i = 0; i < 4; i++)
	{
...
double Vec4::Angle3(const Vec4 & u, const Vec4 & v)
{
	Vec4 uNorm = Normalize3(u);
	Vec4 vNorm = Normalize3(v);
	return acos(Dot3(uNorm, vNorm));
}
...
double Vec4::Length4(const Vec4 & u)
{
...
Vec4 Vec4::Normalize4(const Vec4 & u)
{
	Vec4 v = u / Length4(u);
	return v;
}
...
double Vec4::Distance4(const Vec4 & u, const Vec4 v)
{
	return Length4(u - v);
}
